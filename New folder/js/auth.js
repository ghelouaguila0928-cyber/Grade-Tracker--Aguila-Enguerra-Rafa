...
/* -------- NEW: single source of truth for the wanted role (active tab) -------- */
...
function showSuccessThenHide(ms = 700) {
  ...
  setTimeout(() => { overlayEl.hidden = true; }, ms);
}
...
if (signupForm) {
  signupForm.addEventListener("submit", async (e) => {
    ...
    // ⚠️ MAIN FIX DITO:
    // Unahin ang value. Kung empty (kadalasan issue sa Automotive / Electronics),
    // fallback tayo sa text ng selected option (yung label na nakikita sa UI).
    let course = courseRaw.trim();     // e.g. "Automotive Technology" OR maybe "" kung walang value
    if (!course && courseEl) {
    ...
    try {
      ...
      await setDoc(doc(db, "users", cred.user.uid), {
        ...
        course,                // ✔ exact course (value or label)
        program: course,       // ✔ duplicate for compatibility
        year,
        yearLevel: year,       // ✔ duplicate
        section,
        classSection: section, // ✔ duplicate
    ...
    try {
      ...
      switch (code) {
        ...
        case "auth/wrong-password":
          ...
          bannerMsg = "Incorrect email or password. Please try again.";
          break;
        ...
        case "auth/too-many-requests":
          ...
          bannerMsg = "Your account is temporarily locked due to too many failed attempts. Try again later or reset your password.";
          break;
        case "auth/network-request-failed":
          bannerMsg = "Network error. Check your internet connection and try again.";
          break;
        default:
          if (!email) flag(emailEl, "Enter your email");
          if (!pass)  flag(passEl,  "Enter your password");
          if (email && pass) flag(passEl, "Unable to log in. Please check your credentials.");
...
onAuthStateChanged(auth, async (user) => {
  ...
  showLoading();
  try { await goToDashboard(user); } finally { hideLoading(); }
});
...
function setActiveRole(role) {
  ...
  document.querySelectorAll("[data-only]").forEach(el =>
    el.style.display = (el.dataset.only || "").toLowerCase() === normalized ? "" : "none"
  );
}
tabs.forEach(t => t.addEventListener("click", () => setActiveRole(t.dataset.role)));
...
signupFloat?.querySelectorAll(".close-float").forEach(btn =>
...
document.querySelectorAll(".password-wrapper").forEach(wrapper => {
  ...
  if (btns.length > 1) btns.forEach((b, i) => { if (i > 0) b.remove(); });
  ...
  const updateVisibility = () => btn.classList.toggle("invisible", input.value.trim().length === 0);
  updateVisibility();
...
// ---- Login banner helpers (outside the form) ----
...
function setLoginBanner(message, { persistMs = 0, sticky = false } = {}) {
  ...
  if (message && String(message).trim()) {
    ...
    try { sessionStorage.setItem(LOGIN_BANNER_KEY, message); } catch {}
    ...
    if (persistMs > 0) {
      loginBannerTimer = setTimeout(() => {
        if (!loginBannerSticky) {
          host.hidden = true;
          host.textContent = "";
          try { sessionStorage.removeItem(LOGIN_BANNER_KEY); } catch {}
        }
        loginBannerTimer = null;
      }, persistMs);
    }
...
forgotModal?.querySelectorAll(".close-forgot").forEach(btn =>
  btn.addEventListener("click", () => {
